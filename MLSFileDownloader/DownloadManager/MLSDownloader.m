...
#define MLSDefaultMaxDownloadCount 1
                        ...
                        switch ([reachability currentReachabilityStatus])
                        {
                                ...
                                case ReachableViaWWAN:
                                {
                                        if ( !self.isOnlyUseWifi && self.isAutoResume )
                                        {
                                                ...
                                                [[NSNotificationCenter defaultCenter] postNotificationName:MLSDownloadRefreshDataNotifaction object:weakSelf];
                                        }
                                        else
                                        {
                                                // 仅在wifi下下载
                                       ...
                                       completion:(MLSDownloaderCompletionCallBackBlock)completion
{
        ...
        if ( range.location != NSNotFound )
        {
                ...
                operation.customParaData = customPara;
        }
        else
        {
                // 不是m3u8文件，提供普通文件下载
...
- (void)cancelAllDownloadingOperation
{
        ...
        [self.downloadingInfo enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, MLSDownloadOperation * _Nonnull obj, BOOL * _Nonnull stop) {
                ...
                if ( obj.isExecuting || obj.isReady )
                {
                        [obj paused];
                }
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:MLSDownloadRefreshDataNotifaction object:self];
}
...
- (void)resumeDownloadWithKey:(NSString *)key completion:(MLSDownloaderOperationCallBackBlock)completion
{
        ...
        if (newOperation == nil && completion != nil)
        {
                NSError *error = [NSError errorWithDomain:errorDomain code:DownloadOperationErrorCodeReusmeError userInfo:nil];
                completion(nil,error);
                return;
        ...
        [self.downloadingArray replaceObjectAtIndex:[self.downloadingArray indexOfObject:operation] withObject:newOperation];
...
@end
NSString *MLSDownloadRefreshDataNotifaction = @"MLSDownloadRefreshDataNotifaction";