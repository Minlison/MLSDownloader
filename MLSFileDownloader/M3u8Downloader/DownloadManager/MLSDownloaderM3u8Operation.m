...
- (void)checkLocalDownloadState {
    ...
    // 遍历已经下载过的ts视频
    [fileManager enumratePath:self.fileTsPath options:NSDirectoryEnumerationSkipsSubdirectoryDescendants attributes:@{MLSFileManagerEnumrateTypeKey : @"ts"} completionBlock:^(NSString *fileUrl, NSString *fileName, NSError *error) {
    ...
    if ( tsDownloadError == NO && !self.isSuspend && self.currentDownloadTsIndex <= self.totalTsCount) {
        ...
        if (self.waitingDownloadArray.count > 0) {
            ...
            void (^progressBlock)(NSURLSession *session, NSURLSessionDownloadTask *downloadTask, int64_t bytesWritten, int64_t totalBytesWritten, int64_t totalBytesExpectedToWrite)  = ^(NSURLSession *session, NSURLSessionDownloadTask *downloadTask, int64_t bytesWritten, int64_t totalBytesWritten, int64_t totalBytesExpectedToWrite)  {
                ...
                if ((tempCountNetworkSpeed == self.downloadLength || tempCountNetworkSpeed == 0) && self.networkSpeedCallBackBlock != nil)
                {
                    self.networkSpeedCallBackBlock(bytesWritten);
                }
                
                self.downloadLength += bytesWritten;
                ...
                NSLog(@"%@下载进度-----------%f", self.fileName , percent);
                
                percent = percent > 1.0 ? 0.9 : percent;
                
                dispatch_async(dispatch_get_main_queue(), ^{
                    
                    
                    if (strongSelf.progressCallBackBlock != nil && self.isCanProgressCallBack)
                    {
    ...
    if (self.networkSpeedCallBackBlock != nil)
    {
        if (self.networkSpeedCallBackBlock != nil)
        {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                {
                    self.canProgressCallBack = YES;
                    self.networkSpeedCallBackBlock(self.downloadLength - tempCountNetworkSpeed);
                }
                tempCountNetworkSpeed = (int)self.downloadLength;
            });
